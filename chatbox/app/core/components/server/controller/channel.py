...
class ControllerChannel(BaseController):
	...
	def list_joined(self, client_conn: objects.Client, payload: ServerMessageModel) -> None:
		_c.remove_chat_code_from_payload(_c.Codes.CHANNEL_LIST_JOINED, payload)  # noqa

		channels: list[ChannelModel] = self.chat.repo_channel.list_user_joined(client_conn.user.id)
